describe Turnip::StepDefinition do
  ...
  context "turnip-way step description" do
  ...
  context "cucumber-way (regexp) step description" do

    describe ".find" do
      it "returns a step definition that matches the regexp description" do
        all_steps << Turnip::StepDefinition.new(%r{there are ([\d]+) monsters})
        Turnip::StepDefinition.find(all_steps, "there are 23 monsters").expression.should eq(%r{there are ([\d]+) monsters})
      end
    end
    ...
    describe ".execute" do
      ...
      it "executes a step in the given context" do
        all_steps << Turnip::StepDefinition.new(%r{there are ([\d]+) monsters}) { @testing = 123 }
      ...
      it "sends along arguments" do
        all_steps << Turnip::StepDefinition.new(%r{there are ([\d]+) monsters}) { |count| @testing = count.to_i }
        Turnip::StepDefinition.execute(context, all_steps, stub(:description => "there are 23 monsters", :extra_arg => nil))
        context.instance_variable_get(:@testing).should == 23
      end

      it "sends along extra arguments" do
        all_steps << Turnip::StepDefinition.new(%r{there are ([\d]+) monsters}) { |count, extra| @testing = extra }
